...
func (sm *StateManager) callInternal(ctx context.Context, msg *types.Message, priorMsgs []types.ChainMsg, ts *types.TipSet, stateCid cid.Cid,
	...
	if checkGas {
		// When skipSenderValidation is true, use ApplyImplicitMessage which bypasses
		// FVM sender validation. This allows simulating calls from contract addresses
		// or non-existent addresses, matching Geth's skip sender check behavior.
		if skipSenderValidation {
			...
			if err != nil {
				...
				// ret is non-nil but there was an error - continue to get gas info
			}
			// ApplyImplicitMessage may return ret with nil GasCosts since implicit messages
			// don't track gas costs the same way. Only call MakeMsgGasCost if GasCosts is set.
...
func (sm *StateManager) maybeModifySenderForSimulation(
	...
	// The actor is not a valid sender (e.g., it's a contract/EVM actor).
