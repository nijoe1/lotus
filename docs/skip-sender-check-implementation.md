...
go test -v ./itests -run "TestEthCallFromContractAddress|TestEthCallFromNonExistentAddress|TestEthEstimateGasFromContract|TestEthCallFromEOAStillWorks|TestEthCallFromNonExistentAddressWithValue|TestEthCallFromNilAddress|TestEthCallFromNonExistentAddressWithValueDetailed|TestEthCallContractToContract|TestEthCallWithContractMethodData|TestEthEstimateGasLargeValue|TestEthEstimateGasFromContractWithDataTestEthCallPreservesChainState" -count=1
...
### Call Flow for eth_call (UPDATED)

```
EthCall (gas.go)
    └─> ToFilecoinMessage (eth_types.go)
    └─> applyMessage (gas.go)
        └─> StateManager.ApplyOnStateWithGasSkipSenderValidation (call.go)
            └─> callInternal (call.go) [checkGas=true, skipSenderValidation=true]
                └─> vmi.ApplyMessageSkipSenderValidation (vmi.go)
                    └─> FVM.ApplyMessageSkipSenderValidation → delegates to ApplyMessage
                    └─> LegacyVM.ApplyMessageSkipSenderValidation → skips nonce/balance checks
```

### Call Flow for eth_estimateGas (UPDATED)

```
EthEstimateGas (gas.go)
    └─> gasutils.GasEstimateGasLimitSkipSenderValidation (gasutils.go)
        └─> GasEstimateCallWithGasSkipSenderValidation (gasutils.go)
            └─> StateManager.CallWithGasSkipSenderValidation (call.go)
                └─> callInternal (call.go) [checkGas=true, skipSenderValidation=true]
...
1. **VM Interface**: Added `ApplyMessageSkipSenderValidation` method to `vm.Interface`
2. **LegacyVM**: Skips nonce validation and allows execution from non-existent/synthetic actors
3. **FVM/dualExecutionFVM**: Delegates to standard `ApplyMessage` (FVM handles validation internally)
4. **StateManager**: Added `CallWithGasSkipSenderValidation` for gas estimation path
...
| File                                                                | Change                                                                                                                                                           | Status  |
| ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| [chain/vm/vmi.go](../chain/vm/vmi.go)                               | Added `ApplyMessageSkipSenderValidation` to VM `Interface`                                                                                                       | ✅ Done |
| [chain/vm/vm.go](../chain/vm/vm.go)                                 | Implemented `ApplyMessageSkipSenderValidation` on `LegacyVM` - skips nonce/balance checks for simulation                                                         | ✅ Done |
| [chain/vm/fvm.go](../chain/vm/fvm.go)                               | Implemented `ApplyMessageSkipSenderValidation` on `FVM` and `dualExecutionFVM` (delegates to `ApplyMessage`)                                                     | ✅ Done |
| [chain/vm/execution.go](../chain/vm/execution.go)                   | Added `ApplyMessageSkipSenderValidation` to `vmExecutor` wrapper                                                                                                 | ✅ Done |
| [chain/stmgr/call.go](../chain/stmgr/call.go)                       | Added `skipSenderValidation` parameter to `callInternal`, added `CallWithGasSkipSenderValidation` and `ApplyOnStateWithGasSkipSenderValidation` methods          | ✅ Done |
| [node/impl/eth/api.go](../node/impl/eth/api.go)                     | Added `CallWithGasSkipSenderValidation` and `ApplyOnStateWithGasSkipSenderValidation` to StateManager interface                                                  | ✅ Done |
| [node/impl/eth/gas.go](../node/impl/eth/gas.go)                     | Updated `EthEstimateGas` to use `GasEstimateGasLimitSkipSenderValidation`, updated `applyMessage` to use `ApplyOnStateWithGasSkipSenderValidation`               | ✅ Done |
| [node/impl/gasutils/gasutils.go](../node/impl/gasutils/gasutils.go) | Added `GasEstimateCallWithGasSkipSenderValidation` and `GasEstimateGasLimitSkipSenderValidation` functions, added `CallWithGasSkipSenderValidation` to interface | ✅ Done |
| [itests/eth_skip_sender_test.go](../itests/eth_skip_sender_test.go) | New integration tests for skip sender check feature                                                                                                              | ✅ Done |
