...
| ------------------------------------------------------------------- | ------------- | ------------------- |
| [chain/stmgr/call.go](../chain/stmgr/call.go)                       | +143 / -38    | Core implementation |
| [node/impl/eth/api.go](../node/impl/eth/api.go)                     | +3            | Interface update    |
| [node/impl/eth/gas.go](../node/impl/eth/gas.go)                     | +19 / -3      | Consumer of new API |
| [itests/eth_skip_sender_test.go](../itests/eth_skip_sender_test.go) | +200 (new)    | Tests               |
...
| ---------------- | ------------------------ | ----------------------------------------- |
| `EthCall`        | node/impl/eth/gas.go:253 | **CHANGED** - Now uses skip validation    |
| `EthEstimateGas` | node/impl/eth/gas.go:184 | **CHANGED** - Has fallback path           |
| `ethGasSearch`   | node/impl/eth/gas.go:328 | Uses `GasEstimateCallWithGas`, unaffected |

### 6.2 Indirect Consumers (via callInternal)

These functions use `callInternal` with `skipSenderValidation=false`, so behavior is **unchanged**:

| Function                             | File                               | skipSenderValidation               |
| ------------------------------------ | ---------------------------------- | ---------------------------------- |
| `StateManager.Call`                  | chain/stmgr/call.go:44             | false (via CallOnState)            |
| `StateManager.CallWithGas`           | chain/stmgr/call.go:82             | false                              |
| `StateManager.CallAtStateAndVersion` | chain/stmgr/call.go:97             | false                              |
| `GasEstimateCallWithGas`             | node/impl/gasutils/gasutils.go:131 | false (via CallWithGas)            |
| `GasEstimateGasLimit`                | node/impl/gasutils/gasutils.go:160 | false (via GasEstimateCallWithGas) |
...
eth_call / eth_estimateGas (RPC)
    │
    └─▶ ethGas.EthCall / ethGas.EthEstimateGas
            │
            └─▶ ethGas.applyMessage
                    │
                    └─▶ StateManager.ApplyOnStateWithGasSkipSenderValidation [NEW]
                            │
                            └─▶ StateManager.callInternal (skipSenderValidation=true)
                                    │
                                    ├─▶ Create synthetic actor (if non-existent)
                                    ├─▶ Modify actor code (if contract)
                                    └─▶ vmi.ApplyImplicitMessage
```
